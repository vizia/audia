...
pub fn create_playlist_dialog(
    ...
    Binding::new(cx, show_create_playlist_modal, move |cx| {
        if show_create_playlist_modal.get() {
            Window::popup(cx, true, move |cx| {
                VStack::new(cx, |cx| {
                    ...
                    Textbox::new(cx, create_playlist_name)
                        ...
                        .on_edit(|cx, value| {
                            cx.emit(PlaylistsEvent::SetCreatePlaylistName(value))
                        })
                        .on_submit(|cx, _value, enter_key| {
                            if enter_key {
                                cx.emit(PlaylistsEvent::SubmitCreatePlaylist);
                            }
                        })
                        .width(Stretch(1.0));
                    ...
                    HStack::new(cx, |cx| {
                        Button::new(cx, |cx| Label::new(cx, Localized::new("create")))
                            .on_press(|cx| cx.emit(PlaylistsEvent::SubmitCreatePlaylist))
                            .disabled(is_creating_playlist)
                            .width(Pixels(120.0));

                        Button::new(cx, |cx| Label::new(cx, Localized::new("cancel")))
                            .variant(ButtonVariant::Secondary)
                            .on_press(|cx| cx.emit(PlaylistsEvent::CloseCreatePlaylistModal))
                            .disabled(is_creating_playlist)
                            .width(Pixels(120.0));
                    })
                    .gap(Pixels(12.0))
            ...
            .on_close(|cx| cx.emit(PlaylistsEvent::CloseCreatePlaylistModal))
