...
use crate::ui::events::{OAuthEvent, SystemEvent};
            ...
            async move {
                ...
                .save()
                {
                    if let Err(lock_err) = set_oauth_in_progress(&backend, false) {
                        let _ = proxy.emit(SystemEvent::Error(lock_err));
                    }
                ...
                let _ = proxy.emit(OAuthEvent::BrowserOpened);
                let _ = proxy.emit(SystemEvent::StatusMessage(
        ...
        Task::new(move |_| {
            ...
            async move {
                ...
                match (cid, rt) {
                    ...
                    _ => {
                        let _ = proxy.emit(SystemEvent::Error(
    ...
    cx.add_task(
        Task::new(move |_| {
            ...
            async move {
                ...
                let _ = proxy.emit(OAuthEvent::LoggedOut);
                Ok::<(), String>(())
            }
        })
        .name("reset-login")
