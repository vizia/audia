    ...
    cx.add_task(
        Task::new(move |_| {
            ...
            if let Err(err) = webbrowser::open(&url) {
                ...
                let _ = proxy.emit(SystemAppEvent::Error(format!("Failed to open browser: {err}")));
                return Ok::<(), String>(());
            }
            ...
            let code = match code_result {
                ...
                Err(err) => {
                    ...
                    let _ = proxy.emit(SystemAppEvent::Error(format!("OAuth callback error: {err}")));
                    return Ok::<(), String>(());
                }
            };
            ...
            match oauth_api::exchange_code(&client_id, &code, &code_verifier).await {
                Ok(tokens) => {
                    if let Err(err) = apply_token_response(&backend, &tokens, &client_id, &mut proxy).await {
                        let _ = proxy.emit(SystemAppEvent::Error(err));
                    }
        ...
        Task::new(move |_| {
            ...
            match (cid, rt) {
                (Some(cid), Some(rt)) => match oauth_api::refresh_access_token(&cid, &rt).await {
                    Ok(tokens) => {
                        if let Err(err) = apply_token_response(&backend, &tokens, &cid, &mut proxy).await {
                            let _ = proxy.emit(SystemAppEvent::Error(err));
                        }
                    }
                    Err(err) => {
                        let _ =
                            proxy.emit(SystemAppEvent::Error(format!("Token refresh failed: {err}")));
                    }
                },
                _ => {
